...
while ($true) {
    ...
    switch ($choix) {
        ...
        "1" {
            Log "Redemarrage_De_La_Machine"
            $redemarre = Read-Host "Voulez-vous vraiment redemarrer la machine ? (O/N)"
        ...
        "2" {
            ...
            while ($true) {
                ...
                if ([string]::IsNullOrWhiteSpace($IP_cible)) {
                    Write-Host "Vous n'avez pas entre d'adresse IP, reessayez."
                    continue
                }
                if ($IP_cible -notmatch '^(\d{1,3}\.){3}\d{1,3}$') {
                    Write-Host "Format d'adresse IP invalide, reessayez."
                    continue
                }
                break
            }
            ...
            while ($true) {
                ...
                if ([string]::IsNullOrWhiteSpace($utilisateur_distant)) {
                    Write-Host "Vous n'avez pas entre de nom d'utilisateur, recommencez."
                    continue
                }
                break
            }

            Write-Host "Test de connectivite vers $IP_cible..."
            if (-not (Test-Connection -ComputerName $IP_cible -Count 3 -Quiet)) {
                Write-Host "Erreur : la machine $IP_cible ne repond pas."
                MenuSecondaire
                return
            }
        ...
        "3" {
            ...
            if ($reponse -eq "O" -or $reponse -eq "o") {
                ...
                $statut = Invoke-Command -ComputerName $REMOTE_PC -Credential $REMOTE_CRED -ScriptBlock {
                    (Get-NetFirewallProfile -Name Public).Enabled
                }
                if ($statut -eq $true) {
                    Write-Host "Vos pare-feux ont bien ete actives."
                } else {
                    Write-Host "Activation du pare-feu impossible."
                }
            } else {
                Write-Host "Activation pare-feu annulee."
            }
            MenuSecondaire
        }
        ...
        "4" {
            ...
            if (Test-Path -Path $script) {
                ...
                if ($reponse -eq "O" -or $reponse -eq "o") {
                    $session = New-PSSession -ComputerName $REMOTE_PC -Credential $REMOTE_CRED
        ...
        "5" {
            ...
            Write-Host "=== Utilisateurs locaux ($REMOTE_PC - Windows) ==="
            Invoke-Command -ComputerName $REMOTE_PC -Credential $REMOTE_CRED -ScriptBlock {
