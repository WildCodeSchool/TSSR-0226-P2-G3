...
mask_client=$(ssh $ssh_client "powershell.exe -Command '\$prefix = (Get-NetIPAddress -AddressFamily IPv4 | Where-Object {\$_.InterfaceAlias -notlike ''*Loopback*''}).PrefixLength; \$mask = [uint32]0; for(\$i=0;\$i -lt \$prefix;\$i++){\$mask = \$mask -bor (1 -shl (31-\$i))}; \$bytes = [BitConverter]::GetBytes(\$mask); [Array]::Reverse(\$bytes); ([System.Net.IPAddress]\$bytes).ToString()'")
